...
def gen(SbjId):
    ...
    col_names = ['episode', 'trial_id', 'stimCat', 'stimId', 'stimImage', 'filePath', 'bkSwProb', 'itSwProb', 'trialType', 'task', 'response']
    ...
    for bk in range(3):
        ...
        for i in range(len(stimSet)):
            ...
            condE.at[:, 'filePath'] = '../image/' + stimImage[i] + '.jpg'
        ...
        while 1:
            ...
            print(sum(x <= 1 for x in abs(np.subtract(cnt, expVal))))
            print((stimCheck == 0).sum() / len(E))
