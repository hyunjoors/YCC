...
subjectfile = 'data\\MixedCCL_practice' + expInfo['participant'] + '.csv'
...
instr_text = visual.TextStim(win=win, name='instr_text',
    text='This is an instruction page. Please change the text in \'instr_text\' in the main script.',
    font='Arial',
...
for thisEpisode_loop in range(3):
    ...
    M_current_episode = M.loc[M.episode == 'Low'] # <<- 'Low' needs to be changed
    ...
    for i, currentTrial in M_current_episode.iterrows():
        ...
        # update component parameters for each repeat
        trial_image.setImage(currentTrial.filePath)
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if trial_image.status == STARTED:  # only update if drawing
                trial_image.setImage(currentTrial.filePath, log=False)
            ...
            if trial_keyresp.status == STARTED:
                ...
                if len(theseKeys):
                    ...
                    if "escape" == theseKeys:
                        endExpNow = True
                    M.at[i, 'sbjResp'] = theseKeys.name  # just the last key pressed
                    M.at[i, 'sbjRT'] = theseKeys.rt
                    # was this 'correct'?
                    if (trial_keyresp.keys == str(currentTrial.response)) or (trial_keyresp.keys == currentTrial.response):
                        M.sbjCorr = 1
