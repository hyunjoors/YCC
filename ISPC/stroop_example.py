...
M = pd.read_csv('trialData' + os.sep + 'trials_stroop_S' + str(int(expInfo['participant'])) +'.csv') # M.columns tells you the header
M.loc[:,'sbjResp']= None
M.loc[:,'sbjRT']  = np.nan
M.loc[:,'sbjACC'] = np.nan
M.loc[:,'actualOnset']  = np.nan
...
for trialCNT in range(0,len(M),1):
    ...
    event.clearEvents(eventType='keyboard')
    continueRoutine = True
    routineTimer.add(M.loc[trialCNT,'ITIduration']) # This is countdown timer
    while continueRoutine:        
    ...
    # Break #############################################
    if (trialCNT>1) & (trialCNT%160 == 0) & (trialCNT<len(M)):    # total =160 x 3 repetition
        Instruction.setText("Take a break here. \nPress Space to continue")
        currentBkType = 0 # so that the next run starts with a new block instruction
        ins_response = event.BuilderKeyResponse()
        ins_response.status == NOT_STARTED
        Instruction.setAutoDraw(True)
        continueRoutine = True
    ...
    if M.loc[trialCNT,'blockType']!=currentBkType:
        currentBkType = M.loc[trialCNT,'blockType']        
    ...
    M.loc[trialCNT,'actualOnset']  = globalClock.getTime()
        ...
        if response.status == STARTED:
            ...
            if len(theseKeys) > 0:  # at least one key was pressed
                if response.keys == []:  # then this was the first keypress
                    ...
                    M.loc[trialCNT,'sbjRT']   = response.clock.getTime()*1000  # sec -> ms
                    # was this 'correct'?
